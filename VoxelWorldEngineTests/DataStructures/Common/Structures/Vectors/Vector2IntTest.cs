...
public class Vector2IntTest
{
    ...
    {
        Assert.That(Vector2Int.Zero,
            Is.EqualTo(new Vector2Int(0, 0)));
        Assert.That(Vector2Int.One,
            Is.EqualTo(new Vector2Int(1, 1)));
        Assert.That(Vector2Int.UnitX,
            Is.EqualTo(new Vector2Int(1, 0)));
        Assert.That(Vector2Int.UnitY,
    ...
    {
        var v1 = new Vector2Int(1, 2);
        var v2 = new Vector2Int(1, 2);
        var v3 = new Vector2Int(3, 4);
    ...
    {
        var v1 = new Vector2Int(10, 20);
        var v2 = new Vector2Int(10, 20);
    ...
    {
        var v1 = new Vector2Int(1, 2);
        var v2 = new Vector2Int(3, 4);
        Assert.That(v1 + v2,
            Is.EqualTo(new Vector2Int(4, 6)));
    }

    [Test]
    public void Operator_Subtraction()
    {
        var v1 = new Vector2Int(5, 5);
        var v2 = new Vector2Int(2, 1);
        Assert.That(v1 - v2,
    ...
    {
        var v = new Vector2Int(1, -5);
        Assert.That(-v, Is.EqualTo(new Vector2Int(-1, 5)));
    }

    [Test]
    public void Operator_Multiplication_Vector()
    {
        var v1 = new Vector2Int(2, 3);
        var v2 = new Vector2Int(4, 5);
        Assert.That(v1 * v2,
            Is.EqualTo(new Vector2Int(8, 15)));
    }
    ...
    public void Operator_Multiplication_Scalar()
    {
        ...
        // v * int
        Assert.That(v * 2, Is.EqualTo(new Vector2Int(4, 6)));

        // int * v
        Assert.That(3 * v, Is.EqualTo(new Vector2Int(6, 9)));
    }
    ...
    {
        var v = new Vector2Int(10, 20);
        Assert.That(v / 2, Is.EqualTo(new Vector2Int(5, 10)));
    }

    [Test]
    public void Operator_DivisionByZero_ThrowsException()
    {
        var v = new Vector2Int(10, 10);
    ...
    public void Dot_CalculatesCorrectly()
    {
        ...
        var v2 = new Vector2Int(4, 5);
        // 2*4 + 3*5 = 8 + 15 = 23
        Assert.That(Vector2Int.Dot(v1, v2), Is.EqualTo(23));
    }

    [Test]
    public void LengthSquared_CalculatesCorrectly()
    {
        var v = new Vector2Int(3, 4); // 3-4-5 triangle
        // 3*3 + 4*4 = 9 + 16 = 25
    ...
    public void DistanceSquared_CalculatesCorrectly()
    {
        ...
        Assert.That(Vector2Int.DistanceSquared(a, b),
            Is.EqualTo(25));
    }
    ...
    public void Distance_CalculatesCorrectly()
    {
        ...
        var b = new Vector2Int(1, 11); // dist = 10
        Assert.That(Vector2Int.Distance(a, b),
    ...
    {
        var v = new Vector2Int(-5, 10);
        Assert.That(Vector2Int.Abs(v),
            Is.EqualTo(new Vector2Int(5, 10)));
    }
    ...
    {
        var vSmall = new Vector2Int(1, 1); // LenSq = 2
        var vBig = new Vector2Int(5, 5); // LenSq = 50
    ...
    public void IsBetween_WorksCorrectly()
    {
        ...
        // Inside
        Assert.That(new Vector2Int(5, 5).IsBetween(min, max),
            Is.True);

        // Edge cases (Inclusive Start)
        Assert.That(new Vector2Int(0, 0).IsBetween(min, max),
            Is.True);

        // Edge cases (Exclusive End)
        Assert.That(
            new Vector2Int(10, 10).IsBetween(min, max),
            Is.False);

        // Outside
        Assert.That(new Vector2Int(-1, 5).IsBetween(min, max),
            Is.False);
        Assert.That(new Vector2Int(5, 11).IsBetween(min, max),
            Is.False);
    }
}